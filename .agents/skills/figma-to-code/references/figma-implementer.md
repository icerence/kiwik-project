...
4. 새 HTML 페이지를 만들거나 기존 페이지 구현을 마친 직후 `node common/sync-common.mjs`를 실행합니다. 스크립트가 새 페이지를 자동 발견하고 `common/head.html`, `common/header.html`, `common/footer.html`을 적용하므로, 페이지 전용 헤더·푸터를 따로 유지하지 않습니다.
...
3. 대상 페이지에 `common:head`, `common:header`, `common:footer` 생성 영역이 각각 하나씩 있고 공통 원본과 일치하는지 확인합니다.
