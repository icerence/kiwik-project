...
const LIST_URL = `${BASE_URL}/pulmuone/newsroom/listPulmuone.do?menu=313`;
const DELAY_MS = 250;
const MAX_PAGES = 100;
...
async function main(options = {}) {
  ...
  console.log('==================================================');
  console.log('풀무원 뉴스룸 뮤지엄김치간(menu=313) 데이터 수집을 시작합니다.');
  console.log('==================================================');

  const scrapedKimchikanNews = [];
    ...
    try {
      ...
      for (const item of pageItems) {
        if (item.url && !seenUrls.has(item.url)) {
          ...
          scrapedKimchikanNews.push(item);
        }
      }
  ...
  console.log('==================================================');
  console.log(`뮤지엄김치간 총 ${scrapedKimchikanNews.length}건 수집 완료.`);
  ...
  // Add newly scraped museum kimchikan news
  for (const item of scrapedKimchikanNews) {
    if (item.url) {
  ...
  console.log('==================================================');
  console.log(`병합 성공!`);
  console.log(`- 새로 수집된 뮤지엄김치간 뉴스: ${scrapedKimchikanNews.length}건`);
